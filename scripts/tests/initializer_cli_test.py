...
with tempfile.TemporaryDirectory(prefix='ichizen-init-cli-') as tmp, tempfile.TemporaryDirectory(prefix='ichizen-init-receipts-') as receipts:
 ...
 target=json.loads((root/'deploy/gpagoda/database/targets/local-leasing1.json').read_text())
 target['database']={'env_file':'runtime.env','name':name};target['scope']='disposable'
 path=work/'deploy/gpagoda/database/targets/local-leasing1.json';path.parent.mkdir(parents=True);path.write_text(json.dumps(target))
 ...
 env['DB_INIT_ADMIN_PASSWORD']='disposable-fixture-only-password'
 ...
 try:
  ...
  run(['--apply']);env.pop('DB_INIT_ADMIN_PASSWORD');run(['--apply']);run(['--verify'])
