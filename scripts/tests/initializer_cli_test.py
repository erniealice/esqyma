...
def run_case(target_file, target_key, bundle_mode):
 ...
 with tempfile.TemporaryDirectory(prefix='ichizen-init-cli-') as tmp, tempfile.TemporaryDirectory(prefix='ichizen-init-receipts-') as receipts:
  ...
  target=json.loads((root/target_file).read_text())
  target['database']={'env_file':'runtime.env','name':name};target['scope']='disposable'
  if bundle_mode:
   bundle=target['bundles'][0];dest=work/bundle;dest.parent.mkdir(parents=True);dest.write_bytes((root/bundle).read_bytes())
  else:
   target.pop('access',None);target.pop('upgrade',None);target['bundles']=[]
  path=work/target_file;path.parent.mkdir(parents=True);path.write_text(json.dumps(target))
  ...
  case_env=env.copy()
  if bundle_mode:case_env['DB_INIT_ADMIN_PASSWORD']='disposable-fixture-only-password'
  args=[str(binary),'--target',target['target_key'],'--schema-release',target['schema_release']]
  def run(flags):
   p=subprocess.run(args+flags,cwd=work,env=case_env,text=True,capture_output=True)
   if p.returncode:
  ...
  try:
   ...
   run(['--apply'])
   if bundle_mode:case_env.pop('DB_INIT_ADMIN_PASSWORD',None)
...
run_case('deploy/mmis/database/targets/local-education2.json','mmis/local-education2',False)
run_case('deploy/gpagoda/database/targets/local-leasing1.json','gpagoda/local-leasing1',True)
print('Initializer CLI: PASS; education schema-only and leasing bundle targets both passed plan, apply retry, verify, and receipt checks')
