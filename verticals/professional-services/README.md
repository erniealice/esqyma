...
## Universal Job Model — applicability to professional services

The `operation/` proto domain is being generalised under `docs/plan/20260427-universal-job-model/` to handle service / project / maintenance / production work from one schema. Wave 1 (already complete locally) added the enum values `PLANNED`, `RELEASED`, `EQUIPMENT`, `SUBCONTRACT`, `HOLD`, `REWORK`, plus JobTemplate versioning and Job output-target fields. Waves 2–4 add nine new entities. **Professional services is one of the heaviest beneficiaries** of this work — it gets the same plan-vs-actual rigour that manufacturing gets, but framed as engagement-budget management instead of BOM management.

| New entity (Wave) | Relevance to professional services | Example |
|---|---|---|
| `job_template_input` (W2) | 🟢 High — "Engagement requirements" | "Audit engagement template expects 40h Senior Manager + 80h Senior + 120h Staff + $2,500 travel". Reused as the quote-from-template basis when proposing the engagement to a client. |
| `job_template_input_alternate` (W2) | 🟡 Medium — interchangeable senior practitioners by jurisdiction | "Tax review engagement: any CPA in PH-licensed pool; outside PH, any CPA-equivalent credential." |
| `lot` (W2) | ⚪ Low | (low — services aren't lot-tracked) |
| `job_input_plan` (W3) | 🟢 High — **SOW budget freeze at signing** | When an engagement is RELEASED, the planned hours per role are frozen. Subsequent scope creep registers as plan deviations, not as silent budget erosion. |
| `task_interruption` (W3) | 🟡 Medium — "Waiting on client" | An engagement task paused because the client hasn't returned signed source docs — captured with reason and resumption date. Drives utilisation reports. |
| `job_output` (W4) | 🟢 High — deliverables, milestones, signed reports | Audit opinion PDF; tax return; engagement final report. `output_kind=DELIVERABLE` + `deliverable_url`. |
| `job_cost_ledger_entry` (W4) | 🟢 High — T&M and milestone WIP / contract-asset | Unbilled hours roll into contract-asset until invoiced; the ledger is append-only and survives invoice corrections. |
| `job_cost_snapshot` (W4) | 🟢 High — month-end close | Frozen WIP balance per engagement at period close — drives the contract-asset note in the financial statements. |
| `job_plan_deviation` (W4) | 🟢 High — budget vs actual variance | "Engagement consumed 132% of planned senior hours" with reason and approver. Drives engagement profitability reviews. |

**Surface area for professional-services UI:** every existing engagement page (list, detail, timesheet entry, billing) lights up with new tabs and panels — a "Budget" tab showing `job_input_plan` vs. running `job_activity` totals; a "Deliverables" tab listing `job_output` rows; a "Variance" panel surfacing `job_plan_deviation` rows for the engagement partner to review.

**Why this matters:** services firms today track effort and revenue but rarely track "did this engagement consume what we said it would?" with structural rigour. UJM Wave 2–4 supplies the bones; the professional-services facade renders them as engagement budgets, deliverables, WIP, and variance.

**Lyngua tier-3 keys to author** (under `packages/lyngua/translations/en/professional/`):
- `job_template_input.json` → "Engagement requirements" / "Engagement budget line"
- `job_template_input_alternate.json` → "Approved alternate practitioners"
- `job_input_plan.json` → "Frozen engagement budget"
- `task_interruption.json` → "Engagement hold"
- `job_output.json` → "Deliverable"
- `job_cost_ledger_entry.json` → "Engagement WIP ledger"
- `job_cost_snapshot.json` → "Engagement WIP balance"
- `job_plan_deviation.json` → "Budget variance"
