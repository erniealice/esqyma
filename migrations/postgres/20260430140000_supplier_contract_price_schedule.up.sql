-- Migration: supplier_contract_price_schedule (up)
-- Dialect: postgres
-- Created: 2026-04-30T14:00:00Z
-- Plan: docs/plan/20260430-supplier-pricing-symmetry/plan.md (P6 / §6.1 + §6.2)
-- Rationale: Introduces date-windowed pricing under a SupplierContract.
--            Mirrors the sales-side PriceSchedule + PricePlan pair, scoped
--            per contract (suppliers don't share a catalog).
--
--            CRIT-1 (round-6 amendment): overlap detection is enforced by
--            a GIST exclusion constraint over the half-open
--            [date_time_start, date_time_end) range, partitioned by
--            supplier_contract_id, scoped to non-cancelled rows.
--            Requires the btree_gist extension because supplier_contract_id
--            is a TEXT column (not a range type).
--
--            CRIT-2 (round-6 amendment): at most one open-ended row
--            (date_time_end IS NULL) per contract, enforced by a partial
--            unique index. Authoring rules in plan §6.1 detail the
--            "atomic-truncate prior open-ended row" pattern.
--
-- Status enum integer mapping (per proto definition):
--   0 = UNSPECIFIED, 1 = SCHEDULED, 2 = ACTIVE, 3 = SUPERSEDED, 4 = CANCELLED.
