-- Migration: expense_recognition (up)
-- Dialect: postgres
-- Created: 2026-04-30T14:02:00Z
-- Plan: docs/plan/20260430-supplier-pricing-symmetry/plan.md (P8 / §6.3)
-- Rationale: Introduces ExpenseRecognition + ExpenseRecognitionLine — the
--            accrual-basis recognition pair mirroring Revenue + RevenueLineItem.
--
--            HIGH-2 (round-6): the original `(supplier_contract_id, cycle_date)
--            WHERE status='posted'` partial unique key is replaced with a
--            stable, status-independent idempotency_key column. A full
--            UNIQUE INDEX collapses duplicate writes regardless of status.
--
-- Status enum integer mapping (per proto definition):
--   0 = UNSPECIFIED, 1 = DRAFT, 2 = POSTED, 3 = REVERSED.
