-- Migration: accrued_expense (up)
-- Dialect: postgres
-- Created: 2026-04-30T14:03:00Z
-- Plan: docs/plan/20260430-supplier-pricing-symmetry/plan.md (P9 / §6.4 + §6.4a)
-- Rationale: Introduces AccruedExpense — the accrual-direction entity for
--            recognized supplier obligations ahead of bill arrival — and
--            the AccruedExpenseSettlement join table that replaces the
--            original single settling_expenditure_id FK (HIGH-3 amendment).
--
--            Single-write boundary: settled_amount + remaining_amount on
--            AccruedExpense are only updated by the
--            AccruedExpenseSettlementService use case. Adapter-direct
--            writes to those columns are forbidden (plan §10 R2/R3).
--
-- Status enum integer mapping (per proto definition):
--   0 = UNSPECIFIED, 1 = OUTSTANDING, 2 = PARTIAL, 3 = SETTLED, 4 = REVERSED.
